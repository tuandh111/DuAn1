    ...
    SanPhamModel getFormCapNhatLaiSL() {
        ...
        for (SanPhamModel sanPhamModel : listSP) {
            if (sanPhamModel.getMaSP().trim().equals(maSP.trim())) {
                int i = sanPhamModel.getSoLuong() + Integer.parseInt(SoLuongSPBang.replace(".0", ""));
                cd.setSoLuong(i);
                cd.setMaSP(maSP);
            }
        }
...
//            return;
        ...
        try {
            ...
            HDDao.update(model);
            com.DuAn1.Helper.DialogHelper.alert(this, "Thanh toan thanh cong");
            ThaoTacModel ThaoTacModel = getFormThem();
            daoThaoTac.insert(ThaoTacModel);
            int chon = JOptionPane.showConfirmDialog(this, "In hoa don", "Xac nhan hoa don", JOptionPane.YES_NO_OPTION);
            if (chon == 0) {
...
//        }
    ...
    void deleteAll() {
        if (txtMa.getText().equals("")) {
            com.tuandhpc05076.helper.DialogHelper.alert(this, "Bạn có hoa don nào");
            return;
        }
        int chon = tblHoadon.getSelectedRow();
        if (chon < 0) {
        ...
        try {
            HDCTDao.XoaTatCa(txtMa.getText().trim(), maSP.trim());
            filltable();
